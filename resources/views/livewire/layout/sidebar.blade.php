...
>
    ...
    >
        ...
        @if(auth()->user()->can('monitoring'))
            <x-nav-link-sidebar class="{{ request()->routeIs('monitoring.index') ? 'sidebar-active' : '' }}" :href="route('monitoring.index')" :active="request()->routeIs('monitoring.index')" wire:navigate>
            ...
            <x-nav-link-sidebar class="{{ request()->routeIs('monitoring.peta') ? 'sidebar-active' : '' }}" :href="route('monitoring.peta')" :active="request()->routeIs('monitoring.peta')" wire:navigate>
                <svg class="w-5 h-5 mr-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 10V3L4 14h7v7l9-11h-7z"></path></svg>
                Peta Strategis
            </x-nav-link-sidebar>
        @endif
        ...
        @if(auth()->user()->can('logbook-harian') || auth()->user()->can('verifikasi-logbook') || auth()->user()->can('team-saya') || auth()->user()->can('kinerja-bkd'))
            ...
            @if(auth()->user()->can('kinerja-bkd'))
            <x-nav-link-sidebar class="{{ request()->routeIs('kinerja.bkd.*') ? 'sidebar-active' : '' }}" :href="route('kinerja.bkd.index')" :active="request()->routeIs('kinerja.bkd.*')" wire:navigate>
