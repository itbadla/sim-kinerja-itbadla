...
new #[Layout('layouts.app')] class extends Component {
    ...
    public function saveNewRole()
    {
        ...
        // Buat Role
        $role = Role::create(['name' => strtolower($this->newRoleName)]);
                    ...
                    @forelse($roles as $role)
                        <tr class="hover:bg-theme-body/30 transition-colors">
                            ...
                            <td class="px-6 py-4">
                                <div class="flex flex-wrap gap-2">
                                    ...
                                    @else
                                        ...
                                        @empty
                                            <span class="text-xs italic text-theme-muted">Belum ada izin</span>
                                        @endforelse
                                    @endif
                                </div>
                            </td>
                            <td class="px-6 py-4 text-right space-x-2">
                                <!-- TOMBOL EDIT -->
                                <button wire:click="openEditModal({{ $role->id }})" class="text-theme-muted hover:text-primary transition-colors p-2 rounded-lg hover:bg-theme-body border border-transparent hover:border-theme-border" title="Edit Role">
        ...
        <div class="fixed inset-0 z-50 flex items-center justify-center bg-theme-text/20 backdrop-blur-sm transition-opacity px-4">
            <div class="bg-theme-surface rounded-2xl border border-theme-border shadow-2xl w-full max-w-2xl overflow-hidden">
                <div class="px-6 py-4 border-b border-theme-border bg-theme-body/30">
                    <h3 class="text-lg font-bold text-theme-text">Tambah Peran Baru</h3>
                </div>
                <div class="p-6 space-y-6">
                    <div>
                        <label class="block text-xs font-bold text-theme-muted uppercase tracking-wider mb-2">Nama Peran</label>
                        <input type="text" wire:model="newRoleName" class="block w-full border border-theme-border bg-theme-body rounded-xl py-2.5 px-3 text-sm focus:ring-primary focus:border-primary text-theme-text placeholder-theme-muted/50" placeholder="Contoh: kaprodi">
                    ...
                    <div>
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 bg-theme-body p-4 rounded-xl border border-theme-border h-48 overflow-y-auto">
    ...
    @if($isDeleteModalOpen)
        <div class="fixed inset-0 z-50 flex items-center justify-center bg-theme-text/20 backdrop-blur-sm transition-opacity px-4">
            <div class="bg-theme-surface rounded-2xl border border-theme-border shadow-2xl w-full max-w-sm overflow-hidden text-center p-6">
                ...
                <p class="text-sm text-theme-muted mb-6">Apakah Anda yakin ingin menghapus peran <span class="font-bold uppercase">"{{ $roleToDeleteName }}"</span>? Pengguna yang memiliki peran ini mungkin akan kehilangan hak aksesnya.</p>
